...
class ColorSelectionController extends GetxController {
  ...
  _initialAttrSelection(List<Attributes> data, List<ColorsData> colorList) {
    for (var i = 0; i < data.length; i++) {
      insertAttrNameToList(name: data[i].attributeValue![0].value!, index: i);
      insertAttrIdToList(
          id: data[i].attributeValue![0].id!.toString(), index: i);
    }
    _selectedColorName = colorList[0].name!;
    _selectedColorId = colorList[0].id!.toString();
    _generateAttrNames();
    _generateAttrIds();
  }
  ...
  setAttrData(int attrSize, String attrName, String ids, List<Attributes> data,
      List<ColorsData> colorList) {
    _attrSize = attrSize;
    selectedAttrName.value = attrName;
    _generateArray();
    _initialAttrSelection(data, colorList);
  }

  _generateArray() {
    selectedArray = RxList<int>.filled(_attrSize, 0);
    for (var i = 0; i < _attrSize; i++) {
      _selectedAttrNameList.insert(i, "0");
      _selectedAttrIdList.insert(i, "0");
    }
  }
}
